...
const NavLinkItem: React.FC<NavigationItemProps> = ({ data, direction,setOpen }) => {
...
useEffect(() => {
  ...
  return (
    ...
    >
      {data.url.length > 0 ? (
        <NavLink onClick={() => {setIsOpen(false);setOpen()}} className={`flex p-[8px_9px] items-center w-full h-full ${activeClass}`} to={data.url}>
          {data.label}
      ...
      {data.children.length > 0&& (
          ...
          className={`${data.children.length > 0 && !isOpen && "opacity-0 -z-10 hidden overflow-hidden" }  
            ...
            submenu transition-all top-0 right-[-100%] ${isOpen ? "slide-in opacity-100 ${position}" : "opacity-0 slide-out "} 
        ...
        >
          {data.children.map((child, index) => (
            <NavLinkItem key={index} data={child} direction={position} setOpen={setOpen} />
          ))}
        </ul>
      )}
    </li>
...
const NavLinkItem: React.FC<NavigationItemProps> = ({ data, direction, setOpen }) => {
  ...
  return (
    <li
      className={`relative font-light text-sm text-white cursor-pointer max-lg:flex-col flex lg:items-center gap-2 hover:!text-[var(--primary)] `}
      onMouseEnter={() => window.innerWidth > 1000 && setIsOpen(true)}
      onClick={(e) => {
        e.stopPropagation();
        window.innerWidth > 1000 && setIsOpen(!isOpen);
    ...
    >
      {data.url.length > 0 ? (
        <NavLink onClick={() => setOpen()} className={`flex p-[8px_9px] items-center w-full h-full ${activeClass}`} to={data.url}>
          {data.label}
          {data.children.length > 0 && <ChevronDown className="h-3 w-3" />}
        </NavLink>
      ) : (
        <p className={`flex p-[8px_9px] items-center w-full h-full ${activeClass}`}>
          {data.label}
          {data.children.length > 0 && <ChevronDown className="h-3 w-3" />}
        </p>
      )}
      ...
      {data.children.length > 0 && isOpen && (
           <ul style={{
            transform: "translateY(10%)",
          }}
          ref={submenuRef}
          className={`submenu transition-all top-0 right-[-100%] ${isOpen ? "slide-in opacity-100 ${position}" : "opacity-0 slide-out "} 
            dark:bg-[#222] dark:text-white max-sm:ml-4 max-md:!bg-transparent max-md:!text-black max-md:!pl-3 
            lg:absolute lg:bg-white lg:shadow-lg rounded-lg w-[200px] lg:text-gray-700`}
        >
          {data.children.map((child, index) => (
            <NavLinkItem key={index} data={child} direction={position} setOpen={setOpen} />
